...
В этом тесте мы проверяем, что метод `generate` возвращает правильные числа Фибоначчи для различных значений `n`. Мы также проверяем, что метод выбрасывает исключение, если `n` меньше нуля.
